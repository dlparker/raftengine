...
#print(log_control.to_dict_config())
...
async def server_main(uri, transport, cluster_config, local_config, start_pause=False):
    path_root = Path(local_config.working_dir)
    if not path_root.exists():
        path_root.mkdir()
    db_path = Path(path_root, 'raft_log.db')
    module_path = get_module_path('step5', transport)
    module = importlib.import_module(module_path)
    SetupHelper = getattr(module, "SetupHelper")
    helper = SetupHelper()
    raft_server = await helper.get_raft_server(cluster_config, local_config)
    rpc_server = await helper.get_rpc_server(raft_server)
    ...
    await helper.start_server_task(rpc_server)
...
async def main_async(start_pause=True):
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s --transport grpc --port 8080
...
Available transports:
  aiozmq, grpc, fastapi 
    ...
    valid, message = validate_step_transport('step5', transport)
    if not valid:
    ...
    await server_main(uri, args.transport, c_config,  local_config, start_pause=args.startup_pause)
