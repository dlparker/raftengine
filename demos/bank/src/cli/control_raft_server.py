...
async def start_server(index: int, pause, slow_timeouts=False, transport='grpc', base_port=50050) -> bool:
    ...
    server_main_path = Path(__file__).parent.parent / 'step5' / 'raft_ops' / 'server_main.py'
    cmd = [sys.executable, str(server_main_path), '--transport', transport, '--index', str(index), '--base_port', str(base_port)]
    if pause:
...
Available transports:
  grpc, fastapi, aiozmq"""
    ...
    parser.add_argument('--transport', '-t', default='grpc',
    ...
    if args.command == 'start':
        for index in indices:
            await start_server(index, args.startup_paused, args.slow_timeouts, args.transport, args.base_port)
