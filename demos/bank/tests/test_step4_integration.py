#!/usr/bin/env python
"""Integration tests for step4 transport components"""
import pytest
import asyncio
import tempfile
import os
...
# Import step4 client instead of base client for step4 tests
...
def require_dependency(module_name):
    ...
    try:
        ...
        raise ImportError(f"Required dependency '{module_name}' is not installed. Install it to run this step4 integration test.")
...
class TestStep4Integration:
    """Integration tests for step4 component composition"""
    ...
    async def test_aiozmq_component_initialization(self):
        """Test that aiozmq step4 components initialize correctly"""
        require_dependency('aiozmq')
        from step4.aiozmq.setup_helper import SetupHelper
    ...
    async def test_fastapi_component_initialization(self):
        """Test that FastAPI step4 components initialize correctly"""
        require_dependency('fastapi')
        from step4.fastapi.setup_helper import SetupHelper
    ...
    async def test_grpc_component_initialization(self):
        """Test that gRPC step4 components initialize correctly"""
        require_dependency('grpc')
        from step4.grpc.setup_helper import SetupHelper
        from step4.grpc.proxy import ServerProxy
        from step4.grpc.server import BankingServiceImpl
