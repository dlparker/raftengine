...
The final target form of this process will have the client calling the Server API methods via
the ServerProxy to access the Server functions, and the.
...
terms.
   1. A "Collector" class that composes "commands" that indicate the API method to target
      and include the arguments that would normally be sent via an RPC. This needs to be
      in a form that can be serialized to JSON as part of the Raft Append Entries message.
   2. A "Dispatcher" class that takes a "collected" command and arguments, expands it and
      calls the server, and converts the server response into a JSON serializable form.
   3. Build a test that verifies that all the Server API elements work correctly when
      passed through the Collector and Dispatcher.
   4. Build a direct in-process use of Client, Proxy, Colector, Dispatcher and Server,
      because it can be difficult to debug these components with an RPC layer between
      the Proxy and the Collector.
...
5. Build a ProxyWrapper class that implements your application's ServerProxyAPI that converts
...
6. Build a Dispatcher class that decodes your messages into specific server calls and returns the result to
      the caller.
7. Build a special TestFacade that implements ServerProxyAPI that will only be used for
