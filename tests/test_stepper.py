...
async def test_election_steps_1():
    ...
    ps = PhaseStep(node_1.uri, runner_class=action_1, description=desc)
    ...
    ps = PhaseStep(node_2.uri, runner_class=action_2, description=desc)
    phase_2_steps.append(ps)
    ps = PhaseStep(node_3.uri, runner_class=action_2, description=desc)
    phase_2_steps.append(ps)
    ...
    phase_7_steps.append(PhaseStep(node_2.uri, do_now_class=NoOp()))
    ...
    ps = PhaseStep(node_1.uri, runner_class=action_8a)
    ...
    ps = PhaseStep(node_2.uri, runner_class=action_8b)
    phase_8_steps.append(ps)
    ...
    desc = "Leader runs until it has handled one of the pending append entries response messages"
    ps = PhaseStep(node_1.uri, runner_class=action_10, description=desc)
    phase_10_steps.append(ps)
    phase_10_steps.append(PhaseStep(node_2.uri, do_now_class=NoOp()))
    phase_10_steps.append(PhaseStep(node_3.uri, do_now_class=NoOp()))
