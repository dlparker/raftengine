...
1.[x] Need to add persistence for votedFor, and write test where follower votes for candi with term X, crashes,
...
1. Find all places where servers.py and tests open the white box and replace them with event
...
3. rough out ideas for cluster class
...
5. Think deep about reporting errors in raft code, and detecting them in testing. Break something
