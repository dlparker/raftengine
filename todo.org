...
3. [] Have self_add just pick a node from cluster config, expecting to get leader_uri back in the change response,
   then update leader_uri and try again.
4. [] Need to update leader commit voting logic to not self count if exit_in_progress so that commit gets majority without
5. [] Need to figure out what it means to abort remove node. Add abort happens when catchup fails. Maybe remove abort
   would only happen when leader crashes with uncommitted, then new leader removes the record (old leader rejoins after
...
6. [] Need to cleanup message_problem_history code and make a better effort at error reporting, see event item above
7. [] Need to add logic the the bcast_pending code in leader to remove out of date records in case a server
   goes down for a while, could retire on arrival of another message from same server, also on election_timeout
8. [] Need to update cluster change calls so that they only return when complete, like command runner. That way
   caller can know that the change is complete. Consider a callback or event instead
9. [] Update follower remove op to wait for commit, and update leader to sent heartbeat to removing server. Follower logic
...
10. [] completely convert substate to events
11. [] refactor servers.py into subunits, setup_log, test_tracing, sim_net, pausing_server, pausing_cluster, pause_triggers, sequences.
