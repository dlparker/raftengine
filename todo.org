...
1. [] Document that built in method requires node to be "offline". Note how you could keep it online
   ...
   Is there any way to make this not happen by asking the leader for max current snapshot index
   that all nodes can apply? That would not be perfect though, as slow server would delay
   snapshotting foreverish. Prolly need to special case slow server by saying it does a full reset/rejoin.
