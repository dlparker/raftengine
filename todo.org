...
2. [] Need to add logic the the bcast_pending code in leader to remove out of date records in case a server
   goes down for a while, could retire on arrival of another message from same server, also on election_timeout
3. [] refactor servers.py into subunits, setup_log, test_tracing, sim_net, pausing_server, pausing_cluster, pause_triggers, sequences.
