...
1. [X] Add logic to raft_server to save cluster config at startup and on every change. Save URI in a file like PID.
2. [] Need to be able to remove a server from cluster without it running. This will be a new API call in deck.
3. [] full/ops needs a wait to trigger take power if cluster settings has long timeouts, probably want some kind
...
4. [] Need a more encompassing tool for local_only clusters, one that has all the needed tools for
...
5. [x] Consider combining cluster_cmd.py with other command line options such as create_cluster.py and prep_server.py
