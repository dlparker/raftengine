...
async def main(parser, args):
    ...
    for op in args.run_ops:
        if op == "list_clusters":
            if args.json:
                print(await cluster_cli.op_list_clusters(return_json=True))
            else:
                await cluster_cli.do_list_clusters()
        elif op == "cluster_status":
            if args.json:
                print(await cluster_cli.op_cluster_status(return_json=True))
            else:
                await cluster_cli.do_cluster_status()
        elif op == "start_servers":
            if args.json:
                print(await cluster_cli.op_start_servers(return_json=True))
            else:
                await cluster_cli.do_start_servers()
        elif op == "stop_cluster":
            if args.json:
                print(await cluster_cli.op_stop_cluster(return_json=True))
            else:
                await cluster_cli.do_stop_cluster()
        elif op == "send_heartbeats":
            if args.json:
                print(await cluster_cli.op_send_heartbeats(return_json=True))
            else:
                await cluster_cli.do_send_heartbeats()
        elif op == "stop_server":
            if args.json:
                print(await cluster_cli.op_stop_server(args.index, return_json=True))
            else:
                await cluster_cli.do_stop_server(args.index)
        elif op == "server_status":
            if args.json:
                print(await cluster_cli.op_server_status(args.index, return_json=True))
            else:
                stats = await cluster_cli.do_server_status(args.index)
        elif op == "log_stats":
            if args.json:
                print(await cluster_cli.op_log_stats(args.index, return_json=True))
            else:
                stats = await cluster_cli.do_log_stats(args.index)
        elif op == "take_snapshot":
            if args.json:
                print(await cluster_cli.op_take_snapshot(args.index, return_json=True))
            else:
                stats = await cluster_cli.do_take_snapshot(args.index)
        elif op == "server_exit_cluster":
            if args.json:
                print(await cluster_cli.op_server_exit_cluster(args.index, return_json=True))
            else:
...
class ClusterMgr:
    ...
    async def find_clusters(self, search_dir="/tmp", return_json=False):
        ...
        clusters_result = await self.list_clusters(return_json=False)
    ...
    async def select(self, name, return_json=False):
    ...
    async def send_heartbeats(self, return_json=False):
        ...
        return self.response_or_json(result, return_json)
    async def start_servers(self, hostname='127.0.0.1', return_json=False):
