...
class ClusterCLI(MyCommander):
    ...
    async def do_find_clusters(self, search_dir="/tmp"):
        ...
        to be out of date."""
        result = await self._logic_find_clusters(search_dir, return_json=False)
        ...
        if result['total_clusters'] > 0:
            print("\nClusters found:")
            for cluster_name, cluster_info in result['clusters_found'].items():
    ...
    async def do_add_cluster(self, port, host='127.0.0.1'):
        ...
        about the cluster."""
        result = await self._logic_add_cluster(port, host, return_json=False)
    ...
    async def do_update_cluster(self, name=None):
        ...
        result = await self._logic_update_cluster(name, return_json=False)
    ...
    async def do_start_servers(self, hostname='127.0.0.1'):
        ...
        hostname"""
        result = await self._logic_start_servers(hostname, return_json=False)
    ...
    async def do_stop_cluster(self):
        ...
        result = await self._logic_stop_cluster(return_json=self.json_output)
    ...
    async def do_take_snapshot(self, index):
        ...
        result = await self._logic_take_snapshot(index, return_json=False)
    ...
    async def do_stop_server(self, index):
        ...
        result = await self._logic_stop_server(index, return_json=False)
    ...
    async def do_server_exit_cluster(self, index):
        ...
        complete"""
        result = await self._logic_server_exit_cluster(index, return_json=False)
    ...
    async def do_send_heartbeats(self):
        ...
        result = await self._logic_send_heartbeats(return_json=False)
    ...
    async def do_create_local_cluster(self, cluster_name, directory="/tmp", force=False):
        ...
        result = await self._logic_create_local_cluster(cluster_name, directory, force, return_json=False)
    ...
    async def _logic_stop_cluster(self, return_json=False):
    ...
    async def _logic_server_exit_cluster(self, index, return_json=False):
    ...
    async def _logic_send_heartbeats(self, return_json=False):
    ...
    async def _logic_find_clusters(self, search_dir="/tmp", return_json=False):
        """Logic for finding clusters by examining directories"""
        try:
            await self.discover_cluster_files(search_dir)
            
            # Get current cluster list
            clusters_result = await self._logic_list_clusters(return_json=False)
            ...
            result = {
                ...
                "clusters_found": clusters_result["clusters"],
    ...
    async def _logic_start_servers(self, hostname='127.0.0.1', return_json=False):
    ...
    async def _logic_add_cluster(self, port, host='127.0.0.1', return_json=False):
    ...
    async def _logic_update_cluster(self, name=None, return_json=False):
        ...
        try:
            ...
            # Get final status
            final_status_result = await self._logic_cluster_status(name, return_json=False)
    ...
    async def _logic_create_local_cluster(self, cluster_name, directory="/tmp", force=False, return_json=False):
